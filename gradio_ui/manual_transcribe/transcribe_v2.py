...
# Config
# --------------------
AUDIO_DIR = Path("manual_transcribe/wavs_20250416_012741")
...
def get_segment_info(file_rel_path):
    ...
    for line in lines[1:]:  # skip header
        ...
        if len(parts) < 5:
            continue
        segment_folder_path, segment_file_path, start_sec, end_sec, duration_sec = parts

        # Extract wavs_20250416_012741 from path like:
        # data/wavs_20250416_012741_splits/./download_7_segment/segment_99
        path_parts = segment_folder_path.split("/")
        segment_folder = None
        segment_file = None

        # Find the wavs_* folder (should contain _splits)
        for part in path_parts:
            if part.startswith("wavs_") and "_splits" in part:
                segment_folder = part.replace("_splits", "")
                break

        # Find the download_* folder (should contain _segment)
        for part in path_parts:
            if part.startswith("download_") and "_segment" in part:
                segment_file = part.replace("_segment", "")
                break
...
def save_transcription(file_rel_path, transcription):
    ...
    transcriptions[str(file_rel_path).replace("\\", "/")] = {
