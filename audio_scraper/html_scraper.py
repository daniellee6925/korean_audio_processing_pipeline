...
class AsyncAudioScraper:
    def __init__(
        ...
        self.semaphore = asyncio.Semaphore(
            max_concurrent
        )  # limits num of concurrent operations
        ...
        self.session: Optional[aiohttp.ClientSession] = (
            None  # hold http session to track stats
        )
    ...
    async def parse_html(self, url: str) -> Optional[BeautifulSoup]:
        ...
        try:
            ...
            soup = await loop.run_in_executor(
                None, BeautifulSoup, html_source, "html.parser"
            )
            return soup
        except Exception as e:
    ...
    def get_audio_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        for tag in candidate_tags:
            for attr in ("src", "href"):
                src = tag.get(attr)
                if src and any(
                    src.lower().endswith(f".{ext}") for ext in self.audio_extensions
                ):
                    if src.startswith("//"):
                        src = "https:" + src
                    audio_urls.add(src)
    ...
    async def download_audio(self, url: str, file_path: Path) -> bool:
        ...
        try:
            ...
            if response:
                async with (
                    response
                ):  # temp open network stream, guaranteed it gets closed
